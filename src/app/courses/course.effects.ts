...
import {AllCoursesLoaded,
        ...
        LessonsPageLoaded,
        LessonsPageCancelled} from './course.actions';
...
export class CourseEffects {
  ...
  loadLessonsPage$ = this.actions$
    .pipe(
      ...
      tap(lessons => console.log('loadLessonsPage$ LessonsPageRequest')),
      mergeMap(({payload}) => this.coursesService
        .findLessons(
          payload.courseId,
        ...
        .pipe(
          catchError(err => {
            console.log('error loading a lessons page, err: ', err);
